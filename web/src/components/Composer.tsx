...
import { ArrowsClockwise, PaperPlaneTilt, Paperclip, Warning, X } from '@phosphor-icons/react'
...
export function Composer({ conv }: { conv: ConversationId }) {
    ...
    <div className="border-t border-line bg-bg">
      ...
      <div className="flex items-end gap-1.5 px-2.5 py-2.5 sm:px-4 sm:py-3">
        <button
          ...
          className="mb-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] text-muted transition-colors hover:bg-surface-2 hover:text-ink active:scale-[0.98]"
        >
          <Paperclip size={19} />
        </button>
        <input
        ...
        <textarea
          ...
          onKeyDown={onKeyDown}
          onPaste={onPaste}
          className="max-h-[132px] min-h-10 flex-1 resize-none rounded-[10px] border border-line bg-bg px-3 py-2 text-[14.5px] leading-relaxed outline-none transition-colors placeholder:text-muted/70 focus:border-primary"
        />
        <button
          aria-label="发送"
          title="发送"
      ...
      <div className="hidden px-4 pb-2 text-[11px] text-muted sm:block">
        Enter 发送 · Shift+Enter 换行 · 粘贴或拖入即可发文件
      </div>
    </div>
  )
}
